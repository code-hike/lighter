export async function loadGrammarFromFile(language) {
  const content = await readFile(language.path);
...
export async function readFile(filename) {
  ...
  let grammarsPath = path.resolve(__dirname, "..", "grammars");
  let filepath = path.resolve(grammarsPath, filename);
  // const result = await fs.readFile(filepath, "utf8");
  const result = null;

  if (!result) {
    // try using resolve/sync
    const resolve = await import("resolve/sync");
    const indexFile = resolve("@code-hike/lighter", { basedir: __dirname });
    let grammarsPath = path.resolve(indexFile, "..", "..", "grammars");
    let filepath = path.resolve(grammarsPath, filename);
