...
export function runTests({ highlight }) {
  ...
  test("highlight terminal", async () => {
    const result = await highlight("Foo", "terminal", "github-dark");
    expect(result).toMatchSnapshot();
  });

  test("highlight terminal ansi codes", async () => {
    const code = `
[42m [30mastro[39m [0m  [32;1mv2.9.2[0m [1mLaunch sequence.[18B[40D[0m
`.trim();
